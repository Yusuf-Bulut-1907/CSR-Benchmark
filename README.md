...
- [**Tests and Exploratory Experiments**](#tests-and-exploratory-experiments)
- [**Authors**](#authors)
...
The [`Sentiment_analyze.py`](./test/Sentiment_analyse.py) script performs a sentiment analysis on the scraped textual content using the VADER sentiment analysis library. It computes sentiment scores at the document level and aggregates them at the company level in order to explore differences in overall tone across companies. The results are exported as a CSV file for inspection.

The [`test_BERT.py`](./test/test_BRET.py) script explores a semantic clustering approach based on Sentence-BERT embeddings. Documents are embedded using a pretrained transformer model, aggregated at the company level, and clustered using K-Means with cosine-based similarity. This experiment is intended to compare dense semantic representations with traditional TF-IDF-based methods.

The [`test_cluster_cosine.py`](./test/test_cluster_cosine.py) script investigates an alternative clustering strategy based on Non-negative Matrix Factorization (NMF). Latent CSR topics are extracted from the TF-IDF matrix and used as low-dimensional, interpretable features for clustering companies. This approach emphasizes interpretability and business-oriented validation of CSR profiles.

The [`test_matrice_tf_idf.py`](./test/test_matrice_tf_idf.py) is used as a diagnostic tool to inspect the global properties of the TF-IDF matrix generated by the text mining pipeline. It provides basic structural information such as the number of documents and features, evaluates the sparsity of the matrix, identifies the most influential terms based on average TF-IDF scores, and checks for missing values. This script was mainly used to validate the quality and consistency of the textual representation before running clustering or network-based analyses.

The script [`text_mining_tf_idf_petit.py`](./test/text_mining_tf_idf_petit.py) corresponds to an experimental variant of the main TF-IDF pipeline, designed to test a more restrictive and CSR-focused textual representation. In this version, the preprocessing stage explicitly emphasizes CSR-related vocabulary through a dedicated lexicon, while aggressively filtering out generic corporate, legal, and technical noise terms. The objective was to assess whether a tighter semantic focus on environmental, social, and governance concepts could improve interpretability and downstream analytical results.

All test scripts can be executed independently from the project root using:
```bash
